...
export const createProduct = async (req, res, next) => {
    try {
        ...
        if (!name || !description || !price || !category || !images || !images.length) {
            return next(new ApiError(400, "All required fields must be provided, including images"));
        }
    ...
    try {
        ...
        if (images && images.length > 0) {
            ...
            if (base64Images.length > 0) {
                newlyUploaded = await uploadMultipleToCloudinary(base64Images, "products");
            }

            updatedImages = [...existingUrls, ...newlyUploaded];

            // Delete images that are no longer present
            const imagesToDelete = product.images.filter((img) => !existingUrls.includes(img));
